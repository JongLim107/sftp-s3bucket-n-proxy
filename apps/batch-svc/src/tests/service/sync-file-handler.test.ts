...
describe("handler", () => {
  ...
  beforeEach(() => {
    ...
    (SftpClient as jest.Mock).mockReturnValue(sftpMock);
  ...
  it("should establish sftp connection and close it in the end", async () => {
    ...
    expect(sftpMock.connect).toHaveBeenCalledWith("testUser");
  ...
  it("should establish sftp and s3 client given correct user name", async () => {
    ...
    expect(sftpMock.connect).toHaveBeenCalledWith(username);
    expect(sftpMock.close).toHaveBeenCalled();
  ...
  describe("SFTP to S3", () => {
    it("should allow SFTP to S3 upload partially success if unable to read some files", async () => {
      ...
      expect(logger.info).toHaveBeenNthCalledWith(1, `sftp: ["file1","file2"], /cpc/out --> sample-bucket/cpc/in`);
      ...
      expect(logger.info).toHaveBeenNthCalledWith(2, "<< upload to s3 done");
  ...
  describe("S3 to SFTP", () => {
    it("should allow S3 to SFTP upload partially success if unable to read some files", async () => {
      ...
      expect(logger.info).toHaveBeenNthCalledWith(3, `s3_bucket: ["file1","file2"], sample-bucket/cpc/out --> cpc/in`);
      ...
      expect(logger.info).toHaveBeenNthCalledWith(4, "<< upload to sftp done");
      expect(logger.error).not.toHaveBeenCalled();
    });
